...
var prefix = "$";
   client.on('message',function(message) { // Leaked by [ @M3a4x ]
      if(message.content === prefix + "inv") {
          ...
          .setTitle('𝔾𝔼𝕂𝕆 𝕌𝕄𝔼')
          .setURL(`https://discordapp.com/api/oauth2/authorize?client_id=532495192348164097&permissions=8&scope=bot`)
           if(message.author.id !== "352533519068823556") return message.reply('**You aren\'t the bot owner.**');
...
client.on("message", message => {
 if (message.content === "$support") {
  const embed = new Discord.RichEmbed()
      ...
      .setColor("RANDOM")
      .setFooter('♥KingBot♥ جميع الحقوق محفوظة 2019 لبوت')  
...
client.on("ready", () => { // كود رينبو
  function lol() {
    client.guilds.get('523624847331426304').roles.find("name", "Rainbow").setColor("RANDOM");
  };
